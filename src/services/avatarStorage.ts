...
export async function persistAvatar(sourceUri:string,previousUri?:string|null){await FileSystem.makeDirectoryAsync(directory,{intermediates:true});const extension=sourceUri.split('.').pop()?.split('?')[0]?.toLowerCase();const safeExtension=extension&&/^(jpe?g|png|webp|heic)$/.test(extension)?extension:'jpg';const target=`${directory}avatar-${Date.now()}.${safeExtension}`;await FileSystem.copyAsync({from:sourceUri,to:target});if(previousUri&&previousUri!==target&&previousUri.startsWith(directory))await FileSystem.deleteAsync(previousUri,{idempotent:true});return target;}
