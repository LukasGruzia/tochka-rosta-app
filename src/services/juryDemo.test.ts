...
describe('jury demonstration mode',()=>{
  it('offers a complete 60–90 second guided scenario',()=>{expect(juryDemoSteps).toHaveLength(14);expect(juryDemoSteps[0][0]).toBe('Профиль и норма');expect(juryDemoSteps[5][0]).toBe('Реакция Ритма');expect(juryDemoSteps.at(-1)?.[0]).toBe('Бюджет и покупки');});
