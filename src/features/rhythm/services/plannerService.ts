...
interface MatchInput { products:Product[];profile:ProfileDraft;target:NutritionResult;mealType:MealType;remaining:{calories:number;proteinG:number;fatG:number;carbsG:number};budget?:BudgetSettings|null;usedProductIds?:number[];useHistory?:boolean; }
export async function getRhythmRecommendations(input:MatchInput){
  ...
  const started=Date.now();const preferenceWeights=input.useHistory===false?{}:await loadProductPreferenceWeights();const result=matchRhythmCombinations(input.products,{...input,preferenceWeights,contextHash});await Promise.all([saveRhythmRecommendations(result),recordMealPlanRun({contextHash,mode:'remainder',candidateCount:input.products.length,durationMs:Date.now()-started,result})]);return result;
}
export async function getRhythmDayPlans(input:{products:Product[];profile:ProfileDraft;target:NutritionResult;budget?:BudgetSettings|null;useHistory?:boolean}){const started=Date.now();const preferenceWeights=input.useHistory===false?{}:await loadProductPreferenceWeights();const result=buildRhythmDayPlans(input.products,{...input,preferenceWeights});await recordMealPlanRun({contextHash:createRhythmContextHash({...input.target,useHistory:input.useHistory}),mode:'daily',candidateCount:input.products.length,durationMs:Date.now()-started,result});return result;}
