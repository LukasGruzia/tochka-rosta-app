...
function VectorRhythm({emotion,action,size}:{emotion:RhythmEmotion;action:RhythmAction;size:number}){
  const {colors}=useTheme();const sleeping=emotion==='sleeping';const surprised=emotion==='surprised';const happy=['happy','motivated','celebrating'].includes(emotion);
  const leftArm=action==='wave'||action==='celebrate'?'M25 87C12 76 7 63 11 55':'M24 88C13 92 8 102 10 111';
  const rightArm=action==='point'||action==='celebrate'?'M95 84C108 74 112 61 108 53':'M96 88C108 91 113 101 111 110';
  return <Svg width={size} height={size*1.12} viewBox="0 0 120 150" accessible={false}>
    ...
    <Path d="M43 133V142" stroke={colors.greenPrimary} strokeWidth="12" strokeLinecap="round"/><Path d="M77 133V142" stroke={colors.greenPrimary} strokeWidth="12" strokeLinecap="round"/>
