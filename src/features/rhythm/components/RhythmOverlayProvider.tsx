...
export function RhythmOverlayProvider({children}:PropsWithChildren){
  ...
  const updateSettings=useCallback(async(next:RhythmSettings)=>{await saveRhythmSettings(next);setSettings(next);if(!next.enabled||next.mode==='off')setQueue([]);},[]);
  ...
  return <RhythmOverlayContext.Provider value={value}>{children}{active&&!keyboardVisible?<View pointerEvents="box-none" style={[styles.overlay,{bottom:Math.max(insets.bottom,spacing.md)+(pathname.includes('(tabs)')||['/','/diary','/catalog','/flow','/profile'].includes(pathname)?88:12)}]}><RhythmReactionToast decision={active} animated={animationsEnabled} onDismiss={dismiss} onOpen={()=>{dismiss();router.push('/rhythm-center');}}/></View>:null}</RhythmOverlayContext.Provider>;
}
