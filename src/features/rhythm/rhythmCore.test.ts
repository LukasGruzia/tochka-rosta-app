...
describe('Rhythm local assistant core',()=>{
  ...
  it('respects independent reaction, recommendation and budget controls',()=>{
    const scenarioContext={...context,route:'/flow'};
    expect(decideRhythmResponse({type:'MEAL_ADDED'},scenarioContext,{...settings,reactionsEnabled:false})).toBeNull();
    expect(decideRhythmResponse({type:'REMAINDER_MATCH_OPENED'},scenarioContext,{...settings,recommendationsEnabled:false})).toBeNull();
    expect(decideRhythmResponse({type:'BUDGET_APPROACHING'},scenarioContext,{...settings,budgetEnabled:false})).toBeNull();
  });
  ...
  it('respects disabled state, cooldown and rejection suppression',()=>{expect(decideRhythmResponse({type:'MEAL_ADDED'},context,{...settings,enabled:false})).toBeNull();expect(decideRhythmResponse({type:'MEAL_ADDED'},{...context,recentRejections:2},settings)).toBeNull();expect(decideRhythmResponse({type:'MEAL_ADDED'},{...context,lastInitiativeAt:new Date().toISOString()},settings)).toBeNull();expect(decideRhythmResponse({type:'DAY_COMPLETED'},context,settings)?.visual.emotion).toBe('celebrating');});
  ...
  it('keeps the bounded planner responsive on a large local catalogue',()=>{const catalogue=Array.from({length:250},(_,index)=>product(index+1));const started=performance.now();const result=matchRhythmCombinations(catalogue,{profile,target,mealType:'lunch',remaining:context.remaining!,contextHash:'benchmark'});const plans=buildRhythmDayPlans(catalogue,{profile,target});expect(result.length).toBeGreaterThan(0);expect(plans.length).toBeGreaterThan(0);expect(performance.now()-started).toBeLessThan(1500);});
});
