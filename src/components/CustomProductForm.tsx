...
export function CustomProductForm({ productId }: { productId?: number }) {
  ...
  useEffect(() => {
    if (!productId) return;
    void getProductById(productId).then((product) => {
      if (!product) return;
      setDraft({
        id: product.id,
        name: product.name,
        ...
        sodiumMg: product.sodiumPer100g,
        allergens: product.allergens,
        barcode: product.barcode,
        imageUri: product.imageUri,
      });
      originalImage.current = product.imageUri;
      setValues({
        basisAmount: "100",
        serving: String(product.servingSizeG),
        package: product.packageSizeG ? String(product.packageSizeG) : "",
        calories: String(product.caloriesPer100g),
        protein:
          product.proteinPer100g == null ? "" : String(product.proteinPer100g),
        fat: product.fatPer100g == null ? "" : String(product.fatPer100g),
        carbs: product.carbsPer100g == null ? "" : String(product.carbsPer100g),
        fiber: product.fiberPer100g == null ? "" : String(product.fiberPer100g),
        sugar: product.sugarPer100g == null ? "" : String(product.sugarPer100g),
        sodium:
          product.sodiumPer100g == null ? "" : String(product.sodiumPer100g),
      });
    });
  }, [productId]);
  useEffect(() => () => {
  ...
  const submit = async () => {
    const normalized: ProductDraft = {
      ...draft,
      basisAmount: Number(values.basisAmount.replace(",", ".")),
      servingSizeG: Number(values.serving.replace(",", ".")),
      packageSizeG: numberOrNull(values.package),
      calories: Number(values.calories.replace(",", ".")),
      proteinG: numberOrNull(values.protein),
      fatG: numberOrNull(values.fat),
      carbsG: numberOrNull(values.carbs),
      fiberG: numberOrNull(values.fiber),
      sugarG: numberOrNull(values.sugar),
      sodiumMg: numberOrNull(values.sodium),
    };
    if (!normalized.name.trim()) return Alert.alert("Добавь название");
    if (!Number.isFinite(normalized.calories) || normalized.calories < 0)
      return Alert.alert("Проверь калорийность");
    try {
      setSaving(true);
      const product = productId
        ? await updateCustomProduct(productId, normalized)
        : await createCustomProduct(normalized);
      pendingImage.current = null;
      if (originalImage.current && originalImage.current !== normalized.imageUri) {
        void deleteStoredFoodImage(originalImage.current).catch(() => undefined);
      }
      originalImage.current = normalized.imageUri ?? null;
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.headerCopy}>
            ...
            <AppText tone="secondary">
              Пищевая ценность останется только на этом устройстве.
            </AppText>
          </View>
        </View>
        <GlassCard style={styles.form}>
          ...
          <FormField
            ...
            placeholder="Например: молоко, орехи"
          />
          <FormField
            label="Заметка"
            value={draft.note ?? ""}
        ...
        <GlassCard style={styles.form}>
          <AppText variant="heading">Основа расчёта</AppText>
          <View style={styles.chips}>
            {(
              [
                ...
                ["serving", "На порцию"],
                ["package", "На упаковку"],
              ] as [NutritionBasis, string][]
            ).map(([basis, label]) => (
              <FilterChip
                key={basis}
                label={label}
                selected={draft.basisType === basis}
                onPress={() => {
                  setDraft({ ...draft, basisType: basis });
                  if (basis === "per100g")
                    setValues((current) => ({
                      ...current,
                      basisAmount: "100",
          ...
          <FormField
            label="Количество для указанных КБЖУ"
            value={values.basisAmount}
            onChangeText={updateValue("basisAmount")}
            keyboardType="decimal-pad"
            suffix="г"
          />
          <FormField
            label="Обычная порция"
            value={values.serving}
            onChangeText={updateValue("serving")}
            keyboardType="decimal-pad"
            suffix="г"
          />
          <FormField
            ...
            onChangeText={updateValue("package")}
            keyboardType="decimal-pad"
            suffix="г (необязательно)"
          />
        </GlassCard>
        <GlassCard style={styles.form}>
          <AppText variant="heading">Пищевая ценность</AppText>
          <FormField
            label="Калории *"
            value={values.calories}
            onChangeText={updateValue("calories")}
            keyboardType="decimal-pad"
            suffix="ккал"
          />
          <View style={styles.columns}>
            <View style={styles.column}>
              <FormField
                ...
                onChangeText={updateValue("protein")}
                keyboardType="decimal-pad"
                suffix="г"
              />
            </View>
          ...
          <FormField
            label="Углеводы"
            value={values.carbs}
            onChangeText={updateValue("carbs")}
            keyboardType="decimal-pad"
            suffix="г"
          />
          <FormField
            label="Клетчатка"
            value={values.fiber}
            onChangeText={updateValue("fiber")}
            keyboardType="decimal-pad"
            suffix="г"
          />
          <FormField
            label="Сахар"
            value={values.sugar}
            onChangeText={updateValue("sugar")}
            keyboardType="decimal-pad"
            suffix="г"
          />
          <FormField
            label="Натрий"
            value={values.sodium}
            onChangeText={updateValue("sodium")}
            keyboardType="decimal-pad"
            suffix="мг"
          />
          <AppText variant="caption" tone="muted">
            Белки, жиры и углеводы можно оставить пустыми — приложение покажет
            «—», а не подставит ноль.
          </AppText>
        </GlassCard>
        <GlassCard style={styles.form}>
          ...
          <PrimaryButton
            label={
              draft.imageUri ? "Заменить фотографию" : "Выбрать фотографию"
            }
            secondary
            onPress={pickImage}
          />
          <FormField
            label="Штрихкод / QR"
            value={draft.barcode ?? ""}
            onChangeText={(barcode) => setDraft({ ...draft, barcode })}
            placeholder="Необязательно"
          />
        </GlassCard>
        <PrimaryButton
          label={saving ? "Сохраняем…" : "Сохранить продукт"}
          disabled={saving}
          onPress={submit}
        />
...
const styles = StyleSheet.create({
  ...
  column: { flex: 1 },
  photo: { width: "100%", aspectRatio: 1.6, borderRadius: radii.md },
});
