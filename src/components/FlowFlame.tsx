...
import { flowFlameConfig, getFlowFlameLevel } from '@/services/flowFlameState';
...
export function FlowFlame({ streak, size = 150 }: { streak: number; size?: number }) {
  ...
  useEffect(() => { pulse.value = reducedMotion ? 0 : withRepeat(withSequence(withTiming(1, { duration: config.speed / 2, easing: Easing.inOut(Easing.sin) }), withTiming(0, { duration: config.speed / 2, easing: Easing.inOut(Easing.sin) })), -1, false); }, [config.speed, pulse, reducedMotion]);
