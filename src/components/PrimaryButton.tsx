...
export function PrimaryButton({ label, onPress, disabled, secondary, accessibilityLabel }: Props) {
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={accessibilityLabel ?? label} disabled={disabled} onPress={handlePress}
      ...
      {secondary ? <View style={styles.secondary}><AppText style={styles.label}>{label}</AppText></View> :
        <LinearGradient colors={[colors.greenBright, colors.greenPrimary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.primary}>
          <AppText style={styles.darkLabel}>{label}</AppText>
        </LinearGradient>}
    </Pressable>
  );
}
const styles = StyleSheet.create({
  ...
  secondary: { minHeight: 52, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.glassBorder, borderRadius: radii.md },
  label: { fontWeight: '700' }, darkLabel: { color: colors.backgroundPrimary, fontWeight: '800' }, disabled: { opacity: 0.4 }, pressed: { transform: [{ scale: 0.985 }], opacity: 0.9 },
});
