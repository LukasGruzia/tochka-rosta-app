...
export function GlassSurface({ children, variant = 'base', selected = false, onPress, style, contentStyle, accessibilityLabel, haptic = 'light' }: Props) {
  ...
  const gradient = isAccent
    ? [colors.surfaceAccent, colors.surfaceRaised] as const
    : [colors.specular, surface] as const;
  const shellStyle = [
    styles.shell,
  ...
  const content = <>
    {Platform.OS === 'ios' && flags.enableAdvancedGlassBlur
      ? <BlurView intensity={level.blur} tint={isDark ? 'dark' : 'light'} style={StyleSheet.absoluteFill} />
      : null}
  ...
  if (onPress) {
    return <AppPressable
      ...
      actionLabel={accessibilityLabel ?? `glass_${variant}`}
      haptic={haptic}
      onPress={onPress}
      style={shellStyle}
