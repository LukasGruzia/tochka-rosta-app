...
export function WaterCard({ onOpen }: { onOpen: () => void }) {
  ...
  const add = async (amount: number) => { await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); await addWater(amount); };
  return <GlassCard variant="compact">
    ...
    <View style={styles.actions}>{[200, 250, 500].map((amount) => <Pressable key={amount} accessibilityRole="button" accessibilityLabel={`Добавить ${amount} миллилитров воды`} onPress={() => { void add(amount); }} style={({ pressed }) => [styles.amount, { backgroundColor: colors.surface, borderColor: colors.glassBorder }, pressed && styles.pressed]}><AppText variant="caption" style={styles.bold}>+{amount}</AppText></Pressable>)}</View>
  </GlassCard>;
}
...
const styles = StyleSheet.create({
  ...
  amount: { flex: 1, minHeight: 38, borderRadius: radii.pill, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, bold: { fontWeight: '700' }, pressed: { opacity: 0.65 },
});
