...
export default function DiaryScreen() {
  ...
  const restoreEntry = useAppStore((state) => state.restoreDiaryEntry);
  const repeatEntry = useAppStore((state) => state.repeatDiaryEntry);
  const addFavorite = useAppStore((state) => state.addFavorite);
  const targetNutrition = useAppStore((state) => state.target);
  const [selected, setSelected] = useState<DiaryEntry | null>(null);
  const [copying, setCopying] = useState(false);
  const [pendingUndo, setPendingUndo] = useState<DiaryEntry | null>(null);
  ...
  return <>
    ...
    {selected ? <EntryEditor entry={selected} onClose={() => setSelected(null)} onSave={async (entry, meal, grams) => { await editEntry(entry.id, meal, grams / entry.servingSizeG, grams); }} onRepeat={async (entry) => { await repeatEntry(entry.id); if (flags.enableHaptics) void safelyRunHaptic('success'); }} onFavorite={async (entry) => { if (!entry.productId) return; await addFavorite(entry.productId); if (flags.enableHaptics) void safelyRunHaptic('success'); Alert.alert('Добавлено в избранное', entry.productName); }} onDelete={(entry) => Alert.alert('Удалить запись?', entry.productName, [{ text: 'Отмена', style: 'cancel' }, { text: 'Удалить', style: 'destructive', onPress: async () => { if (flags.enableHaptics) void safelyRunHaptic('warning'); await removeEntry(entry.id); setSelected(null); setPendingUndo(entry); } }])}/> : null}
    ...
    {pendingUndo ? <UndoToast message="Продукт удалён" onExpire={() => setPendingUndo(null)} onUndo={async () => { await restoreEntry(pendingUndo.id); if (flags.enableHaptics) void safelyRunHaptic('success'); }} /> : null}
  </>;
}

function EntryEditor({ entry, onClose, onSave, onRepeat, onFavorite, onDelete }: { entry: DiaryEntry; onClose: () => void; onSave: (entry: DiaryEntry, meal: MealType, grams: number) => Promise<void>; onRepeat: (entry: DiaryEntry) => Promise<void>; onFavorite: (entry: DiaryEntry) => Promise<void>; onDelete: (entry: DiaryEntry) => void }) {
  const { colors } = useTheme();
  ...
  return <Modal visible transparent animationType="slide" onRequestClose={onClose}><Pressable style={[styles.scrim, { backgroundColor: colors.blackScrim }]} onPress={onClose}/><View style={[styles.sheet, { backgroundColor: colors.surfaceSolid, borderColor: colors.glassBorderStrong }]}><ScrollView contentContainerStyle={styles.sheetContent}><AppText variant="heading">Изменить запись</AppText><AppText tone="secondary">{entry.productName}</AppText><AppText variant="caption" tone="muted">ПЕРЕНЕСТИ В ДРУГОЙ ПРИЁМ</AppText><View style={styles.chips}>{mealOrder.map((item) => <FilterChip key={item} label={mealLabels[item]} selected={meal === item} onPress={() => setMeal(item)}/>)}</View><AppText variant="caption" tone="muted">ИЗМЕНИТЬ ПОРЦИЮ</AppText><View style={[styles.quantity, { backgroundColor: colors.surface }]}><TextInput accessibilityLabel="Количество в граммах" value={quantity} onChangeText={setQuantity} keyboardType="decimal-pad" style={[styles.input, { color: colors.textPrimary }]}/><AppText tone="secondary">г</AppText></View><PrimaryButton label={saving ? 'Сохраняем…' : 'Сохранить изменения'} disabled={saving || grams <= 0} onPress={async () => { setSaving(true);try{await onSave(entry,meal,grams);setSaving(false);onClose();}catch(error){setSaving(false);throw error;} }}/><AppText variant="caption" tone="muted">БЫСТРЫЕ ДЕЙСТВИЯ</AppText><PrimaryButton label="Повторить запись" secondary disabled={saving} onPress={async()=>{setSaving(true);try{await onRepeat(entry);setSaving(false);onClose();}catch(error){setSaving(false);throw error;}}}/>{entry.productId ? <PrimaryButton label="Добавить в избранное" secondary disabled={saving} onPress={async()=>{try{setSaving(true);await onFavorite(entry);}finally{setSaving(false);}}}/> : null}<PrimaryButton label="Удалить запись" secondary disabled={saving} onPress={() => onDelete(entry)}/><PrimaryButton label="Отмена" secondary onPress={onClose}/></ScrollView></View></Modal>;
}
