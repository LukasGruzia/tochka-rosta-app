...
export default function MealPlanScreen() {
  const { diaryDate, mealPlan, generatePlan, loadPlan, resetPlan, addToDiary } = useAppStore(); const [busy, setBusy] = useState(false);
  useFocusEffect(useCallback(() => { void loadPlan(); }, [loadPlan]));
  const generate = async () => { try { setBusy(true); await generatePlan(); } catch (error) { Alert.alert('Не удалось собрать рацион', error instanceof Error ? error.message : 'Попробуй позже'); } finally { setBusy(false); } };
...
const styles = StyleSheet.create({ close: { width: 42, height: 42, borderRadius: radii.pill, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface }, empty: { gap: spacing.lg, alignItems: 'center' }, center: { textAlign: 'center' }, summary: { alignItems: 'center', gap: spacing.xs }, row: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, copy: { flex: 1, gap: spacing.xs }, name: { fontWeight: '700' }, add: { width: 46, height: 46, borderRadius: radii.pill, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenGlow }, done: { opacity: 0.55 } });
