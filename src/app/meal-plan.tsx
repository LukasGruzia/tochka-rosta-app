...
import { PlannerSkeleton, ScreenState } from '@/components/ScreenStates';
...
export default function MealPlanScreen() {
  ...
  const diaryDate = useAppStore((state) => state.diaryDate); const mealPlan = useAppStore((state) => state.mealPlan); const generatePlan = useAppStore((state) => state.generatePlan); const loadPlan = useAppStore((state) => state.loadPlan); const resetPlan = useAppStore((state) => state.resetPlan); const addToDiary = useAppStore((state) => state.addToDiary); const [busy, setBusy] = useState(false);const[quick,setQuick]=useState(false);const[meals,setMeals]=useState<3|4|5>(4);const[mode,setMode]=useState<'mixed'|'budget'|'highProtein'|'quick'>('mixed');const[hydrated,setHydrated]=useState(false);const[loadError,setLoadError]=useState<string|null>(null);const[retryKey,setRetryKey]=useState(0);
  useFocusEffect(useCallback(() => { void retryKey;let active=true;setHydrated(false);setLoadError(null);void loadPlan().catch((error)=>{if(active)setLoadError(error instanceof Error?error.message:'Не удалось загрузить план');}).finally(()=>{if(active)setHydrated(true);});return()=>{active=false;}; }, [loadPlan,retryKey]));
  ...
  return <><TabScreen title="Рацион на день" subtitle={`${diaryDate} · локальная рекомендация`} headerRight={<Pressable style={[styles.close,{backgroundColor:colors.surface}]} onPress={() => router.back()}><AppText>×</AppText></Pressable>}>
...
const styles = StyleSheet.create({ close: { width: 42, height: 42, borderRadius: radii.pill, alignItems: 'center', justifyContent: 'center' }, empty: { gap: spacing.lg, alignItems: 'center' }, center: { textAlign: 'center' }, summary: { alignItems: 'center', gap: spacing.xs }, row: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, copy: { flex: 1, gap: spacing.xs }, name: { fontWeight: '700' }, add: { width: 46, height: 46, borderRadius: radii.pill, alignItems: 'center', justifyContent: 'center' }, done: { opacity: 0.55 },options:{flexDirection:'row',flexWrap:'wrap',gap:spacing.xs,marginTop:spacing.sm} });
