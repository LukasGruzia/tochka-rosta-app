...
export default function FinishScreen() {
  ...
  return <AppBackground><View style={styles.root}><Animated.View entering={reducedMotion ? undefined : ZoomIn.duration(350)} style={styles.check}><AppIcon name="check" size={48} color={colors.backgroundPrimary}/></Animated.View><Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(180)} style={styles.copy}><AppText variant="title">Всё готово</AppText><AppText tone="secondary" style={styles.center}>Твой персональный ритм сохранён на этом устройстве.</AppText></Animated.View></View></AppBackground>;
}
const styles = StyleSheet.create({ root: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: spacing.lg, padding: spacing.lg }, check: { width: 96, height: 96, borderRadius: 48, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.greenBright }, copy: { alignItems: 'center', gap: spacing.sm }, center: { textAlign: 'center' } });
