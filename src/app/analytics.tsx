...
export const ErrorBoundary = createSectionErrorBoundary("AnalyticsScreen");
...
export default function AnalyticsScreen() {
  useRenderTracker("AnalyticsScreen");
  ...
  return (
    <TabScreen
      title="Статистика"
      subtitle="Только данные этого устройства"
      headerRight={
        <Pressable
      ...
      {loadError ? (
        ...
        <>
          <View style={styles.stats}>
            <Stat
              value={Math.round(data.averageCalories).toLocaleString("ru-RU")}
              label="ккал в среднем"
            />
            <Stat
              value={`${Math.round(data.averageTargetAccuracy)}%`}
          ...
          <GlassCard>
            <View style={styles.chartTitle}>
              <AppText variant="heading">Калории по дням</AppText>
              {data.caloriesByDay.length > 30 ? (
                ...
                <View key={item.date} style={styles.barSlot}>
                  <View
                    style={[
                      styles.bar,
                      {
                        height: Math.max(4, (item.calories / max) * 120),
          ...
          <GlassCard>
            <AppText variant="heading">Средние КБЖУ</AppText>
            <View style={styles.macros}>
              <Macro
                label="Белки"
                value={data.averageProteinG}
                color={colors.greenPrimary}
              />
              <Macro
                label="Жиры"
                value={data.averageFatG}
                color={colors.gold}
              />
              <Macro
                label="Углеводы"
                value={data.averageCarbsG}
                color={colors.carbs}
              />
            </View>
          </GlassCard>
          <GlassCard>
            <AppText variant="heading">Приёмы пищи</AppText>
            {(Object.keys(mealLabels) as MealType[]).map((meal) => (
              ...
              >
                <AppText tone="secondary">{mealLabels[meal]}</AppText>
                <AppText>{data.mealDistribution[meal]}</AppText>
              </View>
            ))}
          </GlassCard>
          <GlassCard variant="compact">
            <AppText variant="heading">За период</AppText>
            <AppText tone="secondary">
              Записей: {data.entryCount} · чаще всего:{" "}
              {data.mostFrequentProduct ?? "—"}
            </AppText>
            <AppText tone="secondary">
              Показатели рассчитаны только по дням, где есть записи.
            </AppText>
          </GlassCard>
          <PrimaryButtonLink
            label="Что замечено"
            onPress={() => router.push("/personal-insights" as never)}
          />
          <QuickAddButton onPress={() => setQuick(true)} />
        </>
      )}
      {quick ? (
        <QuickAddSheet
...
function PrimaryButtonLink({
  ...
  return (
    <Pressable onPress={onPress} style={styles.link}>
      <AppText tone="green">{label} →</AppText>
    </Pressable>
  );
}
...
function Stat({ value, label }: { value: string | number; label: string }) {
  return (
    <GlassCard variant="compact" style={styles.stat}>
...
function Macro({
  ...
  return (
    <View style={styles.macro}>
      ...
      <AppText tone="secondary" style={styles.macroLabel}>
        {label}
      </AppText>
      <AppText style={styles.bold}>{Math.round(value)} г</AppText>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  barSlot: { flex: 1, alignItems: "center", gap: 4 },
  bar: { width: "72%", borderRadius: radii.sm },
